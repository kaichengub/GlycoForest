 ...
 * @version sqrt -1
 */
public class PaperExcelExportHelper {

    protected final GraphRepository graphRepository;
    ...
    private final CellStyle redCell;
    ...
    public PaperExcelExportHelper(final GraphRepository graphRepository, final Workbook workbook) {
        ...
        ((XSSFCellStyle) redCell).setFillForegroundColor(new XSSFColor(new java.awt.Color(255, 0, 0)));
        redCell.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        redCell.setVerticalAlignment(VerticalAlignment.TOP);
        redCell.setBorderLeft(BorderStyle.THIN);
    }

    public int addAnnotatedResultList(final Drawing drawing, final Sheet sheet, int rowIndex, int columnIndex, GsmResultList gsmResultList, final IsomorphismType isomorphismType, final Tolerance fragmentTolerance) {

        rowIndex = addExpected(
                workbook, drawing, sheet,
                rowIndex, columnIndex,
                gsmResultList.getSolveTask().getVertex().annotationStream().map(SpectrumAnnotation::getStructure).collect(Collectors.toList()),
                gsmResultList,
                isomorphismType, fragmentTolerance);
        return addResultList(drawing, sheet, rowIndex, columnIndex, isomorphismType, fragmentTolerance, gsmResultList);
    }

    public int addResultList(final Drawing drawing, final Sheet sheet, int rowIndex, int columnIndex, final IsomorphismType isomorphismType, final Tolerance fragTolerance, GsmResultList gsmResultList) {
        ...
        final WithinRunConsensus withinRunConsensus = graphRepository.getSpectrum(((WithinRunStructureVertex) structureVertex).getSpectrumNode());
    ...
    private int addExpected(final Workbook workbook, final Drawing drawing, final Sheet sheet, int rowIndex, int columnIndex, final List<SugarStructure> structures,
                            final GsmResultList gsmResultList, final IsomorphismType isomorphismType, final Tolerance fragmentTolerance) {
        ...
        createCell(sheet, structureRows, columnIndex + 1, columnIndex + 1, "Expected", whiteHeaderCell);

        final CellStyle cellStyle = gsmResultList.isBestCorrect(isomorphismType) ? whiteCell : redCell;

        for (int i = 0; i < structures.size(); i++) {

            final int firstCol = 2 + i * 2;
            addImage(workbook, drawing, sheet, structureRows, columnIndex + firstCol, structures.get(i), cellStyle);
        }
